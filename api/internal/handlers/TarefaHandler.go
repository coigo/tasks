...
func NewTarefaHandler(cfg TarefaHandlerConfig) *TarefaHandler {
	...
	group.GET("/:id/subtarefas", handler.ListarSubtarefas)
...
type CriarTarefaRequest struct {
	...
	TarefaPaiID   *int32   `json:"tarefaPaiId"`
...
func (h *TarefaHandler) ListarSubtarefas(ctx *gin.Context) {
	id, err := strconv.Atoi(ctx.Param("id"))
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"message": "id invalido"})
		return
	...
	subtarefas, err := h.service.ListarSubtarefas(ctx.Request.Context(), int32(id))
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"message": err.Error()})
		return
	}
	ctx.JSON(http.StatusOK, utils.EnsureList(subtarefas))
}
...
func (h *TarefaHandler) Criar(ctx *gin.Context) {
	...
	tarefa, err := h.service.Criar(ctx.Request.Context(), req.Titulo, req.Descricao, req.ProjetoID, criadoPorID, req.ResponsavelID, req.SituacaoID, req.TipoID, req.InicioPrevisto, req.Prazo, req.TarefaPaiID)
	if err != nil {
